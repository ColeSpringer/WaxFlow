...
// The bit layer. WMA Pro is read most significant bit first, bytes in file
...
//
// The shape is codec/wma's, copied rather than imported: the two packages must
// not couple, since a change made for one codec's bitstream must not silently
// alter the other's. The window is wider here (a uint64 rather than four
// bytes), which removes that reader's 25-bit ceiling and with it the class of
// bug where an over-wide shift zero-fills the answer at seven of every eight
// bit positions with no panic and no error.
...
// there and the next boundary are not payload. Reading them back as payload
// lets a walk that should have overrun finish instead, which is the one
// failure this reader exists to prevent.
func (r *bitReader) resetBits(b []byte, n int) {
	r.buf, r.n, r.pos, r.err = b, n, 0, nil
}

// bitLen is the reader's length in bits, and left is what is unread.
...
func (r *bitReader) overrun() uint64 {
	if r.err == nil {
		r.err = malformed("the frame reads past the end of its %d-bit payload", r.n)
	}
	return 0
}
...
func (r *bitReader) bits(n int) uint64 {
	...
	if n < 0 || n > maxBits {
		panic("wmapro: bit field width out of range")
	}
	if r.pos+n > r.n {
		r.pos = r.n
		return r.overrun()
...
// u32 is bits for a field a caller wants as a 32-bit value. It exists so the
// conversion is written once rather than at every call site, where a narrowing
// of the wrong field would be invisible.
func (r *bitReader) u32(n int) uint32 { return uint32(r.bits(n)) }
...
// peek returns the next n bits without consuming them, padded past the end of
// the buffer. The VLC walk needs a fixed-width window and settles the overrun
// itself: a codeword that only decodes because of the padding is caught when
// the walk consumes more bits than remain.
func (r *bitReader) peek(n int) uint64 {
	if n == 0 {
		return 0
	}
	if n > maxBits {
		panic("wmapro: peek wider than the window")
	}
	return (r.window() << (r.pos & 7)) >> (64 - n)
}
...
//
// A frame spans packets and a packet is up to sixteen kilobytes, so this runs
// over most bytes of the stream and its shape matters: the bit-at-a-time loop
// it replaced was an eighth of the decode. The head is filled to a byte
// boundary, the middle is copied a byte at a time with one shift, and only
// the tail takes the general read.
