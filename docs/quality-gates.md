...
- **Validity**: the reference decoder (and ours) accepts every produced
  ...
  signals it (capability matrix), and is delivered without a walk on every
  container: a format that signals its trims per packet is trimmed as the
  packets arrive.
...
| Decoder | Gate |
|---|---|
| FLAC | bit-exact on the full IETF/Xiph suite; the declared total is verified at open in both directions, from one tail read (the Ogg form against the final page granule, the native one against the closing frame); sample-exact seek; >=300x realtime |
| MP3 | vs ffmpeg: RMS < 1e-4 FS, max < 1e-3 FS; LAME gapless sample-count invariant; sample-exact seek at 100 random offsets in VBR; >=150x realtime. The WAV and AIFF-C wrappers decode bit-identically to the elementary stream, frame for frame, and are gated on that rather than on a second ffmpeg run. A truncated stream warns and fails strict mode, on a read as on a walk; the walk confirms the Xing count against the run and, where the run comes up short, settles the reported length to what the packets deliver |
...
| WavPack | bit-exact vs ffmpeg on the official WavPack test suite (every bit depth 8 to 32, the three stereo block modes, redundant-LSB and non-standard-rate files, all four speed modes) and on fixtures from both ffmpeg's encoder and the reference `wavpack` CLI; every out-of-scope shape refused by name (hybrid, float, DSD, more than two channels, and stream versions outside 4.02 to 4.16); the suite's pre-4.0 samples never reach the decoder at all, since they carry the source WAV header and resolve as RIFF; the declared total is verified at open in both directions; sample-exact seek; >=200x realtime |
| PCM (WAV, AIFF, MP4/MOV) | bit-exact vs ffmpeg on every committed fixture, integers compared as int32 words and floats as float32 with **no tolerance at all**: uncompressed audio has nothing for two readers to disagree by, so any difference is a width, byte order, or offset defect rather than rounding. The corpus spans 8/16/24/32-bit integers and 32/64-bit floats, both byte orders of each width, unsigned 8-bit, 5.1, and RF64; in AIFF-C it covers the two compression types that name their own width, `in24` and `in32`, against a file built in the test because no encoder in reach writes one (ffmpeg emits a plain AIFF for 24-bit output, but it does read the spelling, so it is still the oracle); in MP4/MOV it spans both families' spellings (QuickTime `sowt`/`twos`/`NONE`/`raw `, the `in24`/`in32`/`fl32`/`fl64` pair with its `wave`/`enda` byte order, the version 2 `lpcm` flags word, and ISO `ipcm`/`fpcm` with `pcmC`) and the rate a 96 kHz `ipcm` entry can only state through the media timescale. Seek: sample-exact AND bit-identical to the linear decode from every landing, with no pre-roll -- there is no inter-frame state to converge, so anything less is a chunk index addressing the wrong frame. **Writing PCM into an MP4 is a non-goal** (a WAV serves every such use better), so that direction has no gate. Refusals are pinned by name and by error code, the code being the assertion: a storage this build does not unpack is unsupported (non-interleaved or high-aligned LPCM, a width that is not a whole number of bytes, packets wider than their samples, unsigned above 8 bits, a sample depth outside 8/16/24/32), while an entry that contradicts itself is malformed (an `ipcm` without its `pcmC`, a `pcmC` that is not the spec's six bytes, an `ipcm`/`fpcm` width the spec does not define). Damage that the samples survive is a warning `--strict` escalates: an `srat` disagreeing with the 16.16 field, an entry disagreeing with its own WAVEFORMATEX, an LPCM entry claiming more than one frame per packet, a chunk running past the end of the file, and a time-to-sample table whose length disagrees with the bytes the samples occupy. A child box list that does not walk to its end is refused outright, since a box the walk cannot reach is indistinguishable from an absent one and absence is a defined answer for both `enda` and `srat`. **Channel layouts** (MP4/MOV, tracks above stereo): the layout comes from the file's own `chan` or `chnl` box, and the gate is channel identity without an oracle: ten ffmpeg-written fixtures carry one distinct tone per speaker position (the tone keyed to the position, not the channel index), and the channel this tree labels with position P must carry P's tone above every other tone in the file, for a `chan` bitmap, a named tag (`MPEG_5_1_A`, `MPEG_7_1_C`), per-channel descriptions, and `chnl` explicit positions, canonical and permuted alike. The three permuted files (ffmpeg's `channelmap` over the canonical ones, so an independent writer produced the order) decode sample-for-sample equal to their canonical twins, which is the reorder scored against a file nothing here wrote; every well-formed layout file reaches neither a Note nor a warning. **ffprobe agreement**, as masks, on all ten, with one divergence recorded and folded rather than adopted: ffmpeg reads a lone `Ls Rs` pair (a named tag's, or `chnl` positions 4 and 5) as its side pair and reports `5.1(side)`, and this tree reads it as the back pair, because every codec here lands 5.1 on the back-pair mask and so does the WAV mask `container/riff` writes; Apple's own header argues both ways (`WAVE_5_1_B` names the back-pair mask separately, which within that family makes the lone pair the side one, and ffmpeg reads named tags that way), so it is a choice made for the pipeline and the agreement test folds `SL SR` onto `BL BR` for those two rows only. Description labels 5 and 6 need no fold: ffprobe reads them as the back pair too. The oracle's own history is gated by release, measured on builds of 6.1.1, 7.0.2 and 7.1.1: a `chnl` 110 degree pair is refused by 6.1, named `UNK` by 7.0 and placed by 7.1, so the two side-pair `chnl` rows skip the decode differential below 7.0 and the ffprobe agreement below 7.1. Hand-built cells pin the rest by name: every named tag of at most eight channels (permuted ones such as `MPEG_5_1_B`, `MPEG_6_1_A` and `AAC_6_0` yield an order, the WAVE tags none), the pair rule's side clause (`WAVE_6_1`, `WAVE_7_1`, `chnl` positions 8 9 beside 4 5), `chnl` configurations 6 and 12 and an omitted-channels map, both boxes on one entry (the family's own wins, a Note names the other), G.711 and ADPCM adopting a canonical layout and keeping the default with a Note for a permuted one, and the Notes for what is not placed (`DiscreteInOrder`, `SMPTE_DTV`'s `Lt Rt`, an unknown tag, an unmappable label, `chnl` version 1, an object-structured stream, position 126, an unknown position). Refusals under `--strict`, by code: a bitmap or a named tag or a description count or an omitted-channels map that disagrees with the entry's channel count, and a box too short for what it declares. A permuted order rides in the PCM codec config, so the cache key changes only for such a file: an identity config still marshals to the five bytes it always did, pinned in `codec/pcm`. Fuzz: the mp4 target seeds the QuickTime v1/v2 and ISO entries, the hand-built movies that reach the chunk index, the chapter walk and the constant-size `stsz` arm, and a `chan` bitmap, `chan` descriptions, `chnl` positions and a `chnl` configuration with an omitted-channels map. |
| G.711 A-law/mu-law | bit-exact vs ffmpeg on every committed fixture in all three containers (WAV format tags 0x0006/0x0007, AIFF-C `alaw`/`ulaw`, QuickTime `alaw`/`ulaw` fourccs), integers compared as int32 words with **no tolerance at all**: the whole codec is a 256-entry table, so a difference at any code is a wrong table rather than rounding. Both tables are checked at all 256 inputs against a closed form written from the recommendation's own prose, and the four hand-checked values every implementation is verified by (A-law 0xD5 -> +8 and 0x55 -> -8, mu-law's two zero codes) are pinned separately. Seek: sample-exact and bit-identical to the linear decode, since one byte is one sample and there is no state to converge. `bitDepth` reports the stored 8 rather than the decoded 16, which is what `ffprobe` says for the same files and what the waxlabel agreement cells compare against. The `ms` 0x0006/0x0007 spelling has no fixture because no encoder writes one, so it is a hand-built cell instead, which is also the only place the channel count comes from a WAVEFORMATEX rather than the entry header. **Encoding is a non-goal**, so that direction has no gate. Fuzz: the codec target walks arbitrary packets under arbitrary channel counts (there is no bitstream to corrupt, so what it exercises is the channel split, the chunking loop and the ragged-length refusal), and the riff, aiff and mp4 targets seed every container spelling. >=50000x realtime. |
| IMA ADPCM | bit-exact vs ffmpeg on every committed fixture, in both layouts and in all three containers (WAV format tag 0x0011, AIFF-C `ima4`, QuickTime `ima4`; the `ms` 0x0011 spelling is a hand-built cell, no encoder writing one). The two layouts differ in arithmetic and not only in framing, and the difference is measured rather than assumed: the WAV layout uses the multiply form and the QuickTime layout the specification's shift-add form, and swapping them decodes 92 to 99 percent of samples wrong by a few dozen LSB, which reads like a bug elsewhere rather than like the wrong formula. The WAV layout's block LENGTH is measured too, and the format's own samples-per-block formula is wrong about it: `(blockAlign - 4*ch)*2/ch + 1` counts a tail too short for a whole round-robin word, where the reference decodes 9 frames from a 10-byte mono block against the formula's 13 and 1009 from a 1020-byte stereo block against its 1013. Every block align an encoder writes divides evenly, so no fixture can see the difference and a reader that took the formula's answer would diverge from the reference at the first sample past the last whole word. **QuickTime `ima4` carries its predictor across blocks**: a block header restates only its top nine bits, and the reference keeps the running value where the header agrees with it, so a per-block reload sits up to 127 LSB from the linear decode and never rejoins. Consequence, and the deliberate cost: an `ima4` seek lands at the file's start and the engine's pre-roll walks to the target, which is exact and costs about a second per hour of stereo 44.1 kHz audio; the WAV layout resets per block and lands on the block. A fragmented movie carrying either is read: the rule that keeps uncompressed audio out of one is that its `trun` would carry an entry per FRAME, which is false of a 64-frame block, and ffmpeg writes such a file for `-movflags empty_moov`. Length comes from the `fact` chunk where a WAV carries one (the last block decodes past it, and `SamplesExact` is what trims it), believed only where it lands INSIDE that last block, since a count further down disclaims blocks the file coded and a zero or unpatched field would otherwise empty a track; a WAV without one, or with one the demuxer ignored, reports its capacity as exact, because the decode delivers exactly that; and from COMM's packet count in an AIFF-C, which is the field waxlabel 1.8.0 fixed and the one cell where a reader taking it as frames reports 125 samples for 8000. **Encoding is a non-goal**, so that direction has no gate. Fuzz: see the MS row. >=1000x realtime. |
| MS ADPCM | bit-exact vs ffmpeg on every committed fixture (WAV format tag 0x0002), mono and stereo. The QuickTime `ms` 0x0002 spelling has no fixture because no encoder writes one, so it is a hand-built cell: its whole geometry, the predictor table included, lives in a WAVEFORMATEX inside the 'wave' wrapper, and an entry without one is refused by name rather than guessed at. **The file's own predictor coefficient table is used, as the specification says**, and ffmpeg ignores the field and uses the built-in one: a stream with a non-standard table therefore decodes differently there, legitimately, and that is a Note rather than damage so `--strict` does not refuse it. A hand-built cell pins that the table is honoured, by decoding the same bytes twice and requiring the two tables to disagree. The prediction divides by 256 with truncation toward zero, the specification's own expression; an arithmetic shift agrees on every fixture here and differs on a negative odd quotient, so the spec's form is what is implemented. The step size grows by up to 3x per nibble with nothing in the format to bound it, so the state is 64-bit with a documented ceiling and a crafted block sits on the domain floor rather than wrapping to the opposite rail. **Encoding is a non-goal**, so that direction has no gate. Fuzz: the codec target fuzzes the GEOMETRY alongside the payload, which is the interesting half, since the nibble arithmetic is table-bounded and what a crafted file gets to choose is a block size, a channel count and a coefficient table; the run is several blocks through one decoder with no Reset, which is the only way to reach the QuickTime layout's cross-block carry. The riff, aiff and mp4 targets seed every container spelling, including the two "ms" entries whose whole geometry lives in a WAVEFORMATEX. >=1000x realtime. |
...
### Cut (multi-span packet cuts)
- A two-span cut of Opus or AAC to every destination that takes one delivers
  **no energy at all** at the removed span's tone, and both kept spans at full
  level (`TestCutInteriorSpliceKeepsOnlyWhatWasAskedFor`, three tones a third
  apart, measured by single-bin correlation). Before ADR-0011 the join
  delivered a whole codec pre-roll of the removed span: 1024 samples on
  AAC-LC, 3840 on Opus, 24576 on HE-AAC.
- The removed span's bound is `lo/200`, chosen under what the regression
  produces rather than under nothing: a delivered pre-roll measures 2.5e-2 on
  Opus and 7.7e-3 on AAC against kept tones at 0.19, and a clean splice
  measures 1.5e-5 to 5.8e-5. Reverting the policy fails every cell by 8x to
  27x.
- `Landed` is inside the request at every edge, and short of it by under one
  packet per interior edge. Spans that touch are the exception and the reason
  for one: they name one range in two pieces, so they share a boundary and the
  range arrives whole (`TestCutTrackJoinsAdjacentSpansWithoutAHole`).
- With `SpliceTrims` on mka and webm: interior tails land exactly, the decoded
  pre-roll is inaudible, and the decode length equals `plan.Samples`
  (`TestCutSpliceTrimsAreExactOnMatroska`). Every other destination plans nil.
- Browser cells are scripted rather than committed (playwright from the repo's
  `node_modules`, driving `<audio>` and an MSE `SourceBuffer` rather than
  `decodeAudioData`, which bypasses the playback pipeline). Measured
  2026-09-20 on Chromium 1244 and Firefox 153.0: **Chromium plays both shapes
  on both paths** and reports a duration equal to `plan.Samples` (the spliced
  file 24 frames over, the WebM `Duration` element's millisecond rounding);
  **Firefox plays the default shape and rejects the spliced one**, stopping at
  15,616 of 95,352 frames with a media error and raising on `appendBuffer`.
  That rejection is what keeps the exact form opt-in, and it is recorded in
  `docs/client-matrix.md` and ADR-0011.
...
- ffmpeg decodes an encode to **exactly the source's length at lag 0**, for
  CBR and VBR at 44.1 kHz, 48 kHz and 8 kHz (`TestMP3EncodeFFmpegAlignment`,
  scanning 1600 lags in BOTH directions over a frequency-ramp source: the
  two-tone signal the rest of that file uses repeats every 16 samples at
  8 kHz, so every multiple of 16 aligns as well as lag 0 does and the cell
  would prove nothing). Two interop rules sit under that one line, and both
  were broken until 2026-09-20 because every gate scored at the best of 1600
  positive lags:
  - the LAME extension's encoder string has to start `LAME`, `Lavc` or
    `Lavf` or ffmpeg ignores the gapless fields (Firefox takes `LAME` or
    `Lavc`, GStreamer only `LAME`); ours is `LAME WaxF`. mpg123, minimp3,
    stagefright, Rockbox and LAME itself apply them for any string, and
    Media Foundation ignores the tag outright.
  - a two-channel stream must declare the same header mode in every frame.
    ffmpeg reads the mode once, so alternating stereo and joint stereo
    decodes there with the mid/side butterfly applied to frames that never
    had it. The encoder pins joint stereo and flips mode_extension, which
    is LAME's own convention (`TestStereoModeFieldNeverChanges`).
- The tag's counts and both CRCs are written and recomputed in the muxer's
  tests, and the misc byte names the stream's own stereo mode (zero there is
  the field's encoding for MONO, not an absence). LAME's own reader and the
  browser matrix stay manual, and Apple's decoders are unmeasured.
...
G.711 >=**50000x** / IMA and MS ADPCM >=**1000x**;
...
the review round's per-AU headers and gate legs landed). G.711 and the
two ADPCM families were set at their own first bench pass (G.711 338000x, a
256-entry table lookup per byte; IMA WAV 6357x, QuickTime 5359x, MS 5180x,
the QuickTime cell paying for a block header every 64 samples where the
others amortize one over a thousand). The three ADPCM cells gained 10 to 21
percent from carrying the coder state in a local across a run of nibbles and
from writing the specification's truncating division branchlessly, which is
what brings the MS step under the inliner's budget; the same hoist applied to
the MS channel walk made it 2.2x SLOWER, since the channel alternates every
nibble there and the copy costs more than the indirection.
...
- **A linear read through the shared source window costs one window, not one
  per window walked.** `container/internal/srcwin` backs every demuxer that
  scans a stream (flacn, mpa, mka, mp4, wv, apen, mpc, adts, asf), and its
  Trim copied the bytes it kept into fresh storage, discarding the array the
  next extension would have grown into: about two window-sized allocations
  for every 128 KiB of stream, for the length of the stream. Moving the kept
  bytes in place, and growing with `slices.Grow` rather than an append whose
  scratch slice only some targets elide, made the window settle after two
  growths and stay. Measured: a 16 MiB walk 33,159,096 B -> under 524,288 B;
  60-second decodes FLAC 4.43 -> 1.37 MB, MP3 4.95 -> 0.75 MB, ALAC 3.18 ->
  0.61 MB; a 6.3 MB PCM-in-MOV walk 12.5 -> 0.39 MB and 3,243 -> 12,680 MB/s,
  uncompressed audio being the case that moves the most bytes per second.
  Pinned twice, because the loss of it is invisible (every byte still arrives
  correct): on the window's capacity, which holds under `-race`, and on bytes
  off the heap, which does not and is excluded there.
