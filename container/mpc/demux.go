...
func (d *Demuxer) parse() error {
	...
	case string(head[:3]) == "MP+":
		...
		return malformed("Musepack SV%d is not supported: only SV7 and SV8 are", head[3]&0x0F)
	default:
	...
	if err := f.Valid(); err != nil {
		return waxerr.Wrap(waxerr.CodeUnsupportedFormat, "musepack: unusable format", err)
	}
